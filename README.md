...
| **프린트** | PDF·사진·문서를 던져 넣으면 집 서버에 USB로 붙은 프린터로 보냅니다. 사진과 글은 이 Mac에서 고른 용지에 맞춰 PDF로 앉히고(HEIC·한글 텍스트·EUC-KR 포함), 오피스 문서는 LibreOffice가 엽니다. 종이가 몇 장 나갈지 누르기 전에 세어 보여 주고, 서버에는 임시 파일을 남기지 않습니다. **집에서도 밖에서도** 같은 두 주소를 순서대로 시도합니다 |
...
Sources/SeoulLocalAgent/
  ...
  PrintPreparation.swift      사진·글·오피스 문서를 고른 용지의 PDF로 (보내기 전에 이 Mac에서)
