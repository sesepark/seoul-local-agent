...
| 기능 | 내용 |
|---|---|
| **인박스 정리** | Gmail 2계정 · Slack · 메시지 앱 · 캘린더 · 학교 공지 게시판 · **eTL 과목 공지와 과제 마감**을 **읽기 전용**으로 모아 `오늘 꼭 할 일` / `확인해야 할 것` / `기타`로 편집합니다 |
...
| **SO-ARM 101** | 집 서버에 붙은 로봇 팔을 SSH 터널 너머로 조작합니다. 카메라 프리뷰·텔레옵·데이터 수집을 간추린 화면에서 하고, `전체화면`은 서버의 웹 콘솔을 그대로 띄웁니다. **이전 세션이 남긴 토크는 여기서 풉니다** — 끊길 때 토크를 끄지 않는 설계라 다음 텔레옵이 거절되는데, 진단이 토크가 걸린 팔을 찾으면 툴바에 `토크 해제`가 나타납니다(팔이 내려앉을 수 있어 확인을 한 번 받습니다) |
...
| **수집 데이터** | 서버에 쌓인 시연을 에피소드 단위로 되돌려 봅니다. 영상은 내려받지 않고 서버가 그 구간만 잘라 보냅니다. **학습 서버(DGX Spark)로 데이터셋을 보내고 학습된 정책을 회수합니다** — 전송은 콘솔 서버와 학습 서버 사이 LAN으로 흐르므로 맥이 집 밖에 있어도 느려지지 않고(6.2GB 실측 62초/64초), 끊긴 전송은 받다 만 곳부터 이어받습니다 |
| **학습 서버** | DGX Spark의 GPU 하나 앞에 선 줄입니다. 지금 도는 학습의 진행·남은 시간과 대기열, 밤새 끝난 것과 실패 사유를 한 화면에서 봅니다. **여러 개를 세워 두면 앞의 것이 끝나는 대로 하나씩 자동으로 시작하므로, 걸어 두고 자러 가도 됩니다.** LeRobot 정책 학습과 Isaac Lab 강화학습을 걸 수 있고, 걸 수 있는 종류와 그 칸은 서버가 정합니다 — 새 실험을 늘리는 데 앱을 고치지 않습니다. 큐는 Spark 위의 데몬이 들고 있어 이 앱을 꺼도 Mac이 잠들어도 계속 돕니다 |
...
Sources/SeoulLocalAgent/
  ...
  Spark*.swift                학습 서버 작업 큐: 큐 클라이언트, 화면, `--spark-check`
...
- **하드웨어 소유자를 늘리지 않았습니다.** 로봇 팔은 집 서버가 serial과 카메라를 쥐고 있고,
  ...
  실수로 장치 역할이 어긋나는 경로 자체를 없애는 편이 낫기 때문입니다. 서버가 요구하는 확인
  문구(`START SOARM101` 등)는 앱이 채워 보냅니다. 사람에게 남는 게이트는 위험의 종류에 맞춘
  것 하나입니다 — 사람이 리더를 쥐고 있어야 팔이 움직이는 텔레옵·수집은 버튼 한 번, 팔이
  혼자 움직이는 재생과 팔이 힘을 놓는 토크 해제는 체크 한 번입니다.
